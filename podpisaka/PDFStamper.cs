...
namespace podpisaka
{
    public class PDFStamper
    {
        ...
        public static bool AddStamp(string stampFile, FontFamily fontFamily, dkxce.CertificateHash certificateHash, string fileName, PDFSignData signdata)
        {            
            ...
            try
            {
                ...
                if (signdata.Mode == PDFSignData.StampMode.OnlyStamp)
                {
                    ...
                    using (Stream outputPdfStream = new FileStream(tempFile, FileMode.Create, FileAccess.Write))
                    {
                        ...
                        if (!signdata.EachPage) lastAddedPage = Math.Max(lastAddedPage, reader.NumberOfPages - 1);
                        ...
                        if ((!annotated) && modified && signdata.AddAnnot)
                            AddAnnotation(stamper, StampAnnoName, certificateHash.Thumbprint, reader.NumberOfPages, signdata.Offset);
                ...
                {                    
                    using (FileStream outputPdfStream = new FileStream(tempFile, FileMode.Create))
                        modified = AddSignature(fileName, outputPdfStream, certificateHash, image, signdata);
                };
        ...
        private static void AddAnnotation(PdfStamper stamper, string annot_name, string annot_text, int pages, PDFSignData.StampOffset offset)
        {
            ...
            pa.Put(PdfName.KEYWORDS, new PdfString($"Offset={(byte)offset}"));
